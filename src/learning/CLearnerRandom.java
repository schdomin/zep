...
public final class CLearnerRandom
{
    //ds enums
    public static enum ELearnerLabel{ LIKE, DISLIKE, INIT }
    ...
    //ds constructor
    public CLearnerRandom( final CMySQLManager p_cMySQLManager )
    {        
        ...
        System.out.println( "[" + CLogger.getStamp( ) + "]<CLearnerRandom>(CLearnerRandom) CLearnerRandom instance allocated" );
    }
    ...
    public void launch( ) throws SQLException, CZEPMySQLManagerException, MalformedURLException
    {
        ...
        {
            System.out.println( "[" + CLogger.getStamp( ) + "]<CLearnerRandom>(launch) MySQL table is empty - have to wait for crawlers" );
        }
        else
        {
    ...
    public CDataPoint getNextDataPoint( final ELearnerLabel p_eFlag ) throws CZEPEoIException
    {
        //ds save the flag for the last image
        m_mapLabels.put( m_iCurrentImageID, p_eFlag );
        
        //ds check if LIKE/DISLIKE
        if(      ELearnerLabel.LIKE    == p_eFlag ){ ++m_iNumberOfLikes; }
        else if( ELearnerLabel.DISLIKE == p_eFlag ){ ++m_iNumberOfDislikes; }
